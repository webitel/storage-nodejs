...
func (self *SqlSessionStore) Get(sessionIdOrToken string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := self.GetReplica().Select(&sessions, "SELECT 1 as id, 'my-token' as token, '100@10.10.10.144' as userid  FROM session LIMIT 1", map[string]interface{}{}); err != nil {
